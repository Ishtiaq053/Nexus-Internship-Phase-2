...
import { apiGetMeetings } from '../../lib/api';
...
export const EntrepreneurDashboard: React.FC = () => {
  ...
  // Fetch real upcoming meeting count from API
  const loadMeetingCount = useCallback(async () => {
    try {
      const { meetings } = await apiGetMeetings('accepted');
      const now = new Date();
      const upcoming = meetings.filter((m: any) => new Date(m.startTime) > now).length;
          ...
          <CardBody>
            <div className="flex items-center">
              ...
              <div>
                ...
                <h3 className="text-xl font-semibold text-success-900">—</h3>
              </div>
            </div>
          </CardBody>
        </Card>
